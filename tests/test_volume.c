...
int main(void)
{
    ...
    const char *multi_mount_fixture = "{\"blockdevices\":[{\"path\":\"/dev/nvme0n1\",\"type\":\"disk\",\"size\":\"1000\",\"children\":[{\"path\":\"/dev/nvme0n1p1\",\"pkname\":\"nvme0n1\",\"type\":\"part\",\"size\":\"1000\",\"fstype\":\"ext4\",\"mountpoints\":[\"/var/snap/firefox/common/host-hunspell\",\"/\"],\"ro\":false,\"rm\":false}]}]}";
    ...
    assert(volume_parse_lsblk(&inventory, multi_mount_fixture, error, sizeof(error)) == 0);
    assert(inventory.count == 1U);
    assert(strcmp(inventory.items[0].path, "/dev/nvme0n1p1") == 0);
    assert(strcmp(inventory.items[0].mountpoint, "/") == 0);
